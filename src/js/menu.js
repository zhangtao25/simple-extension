import {
  GetLanguageString, MenuCookieEmptyText,
  MenuCookieRootText, MenuCookieUseText,
  MenuRootText, MenuUseragentDefaultText, MenuUseragentRootText,
...
export class Menu {
  ...
  __updateUAMenu(domain) {
    ...
    //检测是否在用默认UA
    chrome.contextMenus.update(this.menus[MenuUseragentDefaultText], {checked: !currentSelected});

    for (let os in UA) {
      const osId = chrome.contextMenus.create({title: os, parentId: this.menus[MenuUseragentRootText]});
      ...
      for (let browser in UA[os]) {
        ...
        chrome.contextMenus.create({
          parentId: osId,
          type: 'checkbox',
          checked: currentSelected ? currentSelected.selected === current : false,
          title: browser,
          onclick: (info, tab) => {
