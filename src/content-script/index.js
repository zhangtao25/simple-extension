        ...
        interceptAJAXRequests = function (namespace) {
            ...
            ah.proxy({
                ...
                onResponse: (response, handler) => {
                    console.log(response.response,1123)
                    for (let i = 0; i < rules.length; i++) {
                        console.log(rules[i].url,handler.xhr.config.url)
                        if (handler.xhr.config.url.includes(rules[i].url)){
                            eval(rules[i].func)
                        }
                    }
