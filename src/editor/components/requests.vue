...
<script>
  import {ConfirmDeleteRequest, GetLanguageString, PromptHowToGetTheRequestUrl} from "../../js/i18_string_name";
  ...
  export default {
    ...
    data() {
      return {
        ui: {
          delete: GetLanguageString(ConfirmDeleteRequest),
    ...
    methods: {
      ...
      async deleteRequest(index, value) {
        if (this.list.length > index) {
          await this.$confirm(this.ui.delete.replace('%value', value));
